...
class auth_plugin_joomdle extends auth_plugin_manual {
    ...
    private function call_method_fgc ($method, $params = '', $params2 = '', $params3 = '' , $params4 = '', $params5 = '') {
        ...
        $response = substr ($response, strripos ($response,'<?xml')); // mahdi agnelli
    ...
    public function get_course_info ($id, $username = '') {
        ...
        $not_found['enddate'] = 0; // mahdi agnelli
        $not_found['lang'] = "";
        ...
        $not_found['teachers'] = array (); // mahdi agnelli
        ...
        foreach ($course_info['teachers'] as $teacher_id => $teacher) {
            $course_info['teachers'][$teacher_id]['coursecounter'] = count( $this->teacher_courses($teacher['username'], 'summary') );
        }
        // } mahdi agnelli
    ...
    public function teachers_abc ($start_chars) {
        ...
        $query = "SELECT
            ...
            AND
            u.suspended=0
            AND $where"; // mahdi agnelli

        $query .= " GROUP BY u.id"; // mahdi agnelli
        ...
        foreach ($records as $p) {
            ...
            $e['lastname']      = $p->lastname;
            $e['username']      = $p->username;
            $e['courseid']      = $p->courseid; // mahdi agnelli
            $e['coursecounter'] = $p->coursecounter; // mahdi agnelli
    ...
    /**
     * mahdi agnelli { @param text $mod, Type of courses information : summary or full }
     */
    public function teacher_courses ($username, $mod) {
        global $CFG, $DB;
        ...
        if ($mod == 'full')
        {
            ...
            $i = 0;
            foreach ($records as $p) {
                $data[$i] = $this->get_course_info($p->remoteid);
                $i++;
            }
        }
        // } mahdi agnelli
        else
        {
            $query = "SELECT
                distinct c.id as remoteid,
                c.fullname, ca.name as cat_name,
                ca.id as cat_id
                FROM
                {$CFG->prefix}course as c,
                {$CFG->prefix}role_assignments AS ra,
            ...
            $query .= " ORDER BY c.id DESC"; // mahdi agnelli
    ...
    public function get_course_mods ($id, $username = '') {
        ...
        foreach ($sections as $section) {
            ...
            foreach ($sectionmods as $modnumber) {
                ...
                $cm = get_coursemodule_from_id(false, $mod->id); // mahdi agnelli
                if ($username) {
                    if (!\core_availability\info_module::is_user_visible($cm, $user->id)) {
