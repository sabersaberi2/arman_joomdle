...
class JoomdleHelperMappings
{
    ...
    static function get_moodle_custom_field_value ($user_info, $field_id, $use_utf8_decode)
    {
        foreach ( $user_info['custom_fields'] as $field)
        {
            if ('cf_'.$field['id'] == $field_id)
            {
                // $data = utf8_decode ($field['data']); // mahdi agnelli
                // mahdi agnelli {
                if ($use_utf8_decode)
    ...
    static function get_user_info_for_joomla ($username)
    {
        ...
        /* Register Date */
        $user_info['registerDate']   = $user->registerDate; // mahdi agnelli
        ...
        switch ($app)
        {
            ...
            default:
                JPluginHelper::importPlugin( 'joomdleprofile' );
                $dispatcher = JDispatcher::getInstance();
                //dump(JoomdleHelperMappings::get_user_info_joomla ($username), "idk!");
                $result = $dispatcher->trigger('onJoomdleGetUserInfo', array($username));
                dump($result, "result");
                foreach ($result as $info)
                {
