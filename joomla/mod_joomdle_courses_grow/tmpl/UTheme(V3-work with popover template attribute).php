...
?>
    ...
    <script type="text/javascript">
        ...
        // var popover_mouseLeave_timeout;
        
        function selfMouseover(_that) {
            // console.log (_that);
            popover_trigger_id = _that.id;
            clearTimeout(window.popover_mouseLeave_timeout);
            console.log('mouseover to popover container : /' + popover_trigger_id + '/ to kept show');
            jQuery(document).ready(function($) {
                // $('div[id^=card-main_m<?php echo $module->id; ?>_]').not(_that).popover('hide');
                $(_that).mouseleave(function() {$.selfMouseLeave(_that);});
            });
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($){
            $('[data-toggle="popover"]').popover({
                html: true,
                content: function() {
                    if (window.console) console.log($('.course-popover-content0').html());
                    return $('.course-popover-content0').html();
                }
            });
        });
    </script>
...
-->
    ...
    <script type="text/javascript">
        ...
        function getID(p) {
            ...
            // console.log('found : /' + found[found.length-1] + '/');
        ...
        jQuery(document).ready(function($) {
            ...
            $('div[id^=card-main_m<?php echo $module->id; ?>_]').popover({
                ...
                html: true,
                placement: 'right',
                container: $('.owl-carousel-joomdlecourses-<?php echo $module->id; ?>'),
                template: '<div id="popover_m<?php echo $module->id; ?>" class="popover" onmouseover="selfMouseover(this)"><div class="arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>',
                content: function() {
                    return $("#popover_container_m<?php echo $module->id; ?>_" + getID(this.id)).html();
                }
            }).click(mouseClick).mouseenter({
                delay_time: 150,
            ...
            // function selfMouseover(_that) {
                ...
                // setTimeout(function()
                    // {
                        // clearTimeout(popover_mouseLeave_timeout);
                        // console.log('mouseover to popover container : /' + popover_trigger_id + '/ to kept show');
                    // }, 120
                // );
                // $(_that).mouseleave(function() {selfMouseLeave(_that);});
            ...
            $.selfMouseLeave = function(_that) {
                popover_trigger_id = 'card-main_m<?php echo $module->id; ?>_';
                window.popover_mouseLeave_timeout = setTimeout(
                    function() 
                    {
                        $('div[id^=' + popover_trigger_id + ']').popover('hide');
                        console.log('mouseLeave to hide popover : /' + popover_trigger_id + '/');
                    }, 90
                );
            }

            // document.addEventListener( 'mouseover', function ( event ) {
                // console.log('# Being mouseover EVENT > ' + popover_trigger_id);
                ...
                // if ( event.target && getClosest(event.target, 'div[id^="popover_dialog_m<?php echo $module->id; ?>_"]') ) {
                    // if (selfMouseenter == false) {
                        // clearTimeout(popover_mouseLeave_timeout);
                        // selfMouseenter = true;
                        // console.log('mouseover to popover container : /' + event.target.id + '/ to kept show');
                    // };
                // } else {
                    // if (selfMouseenter == true) {
                        // console.log('mouseover to other objects from popover container : /' + event.target.id + '/ to hide popover : /' + popover_trigger_id + '/');
                        // // selfMouseenter = false;
                        // $("#" + popover_trigger_id).trigger('mouseleave');
                    // };
                // };
                // console.log('# End mouseover EVENT > ' + popover_trigger_id);
            // });

            function mouseClick(e) {
                $("#" + this.id).popover('toggle');
                console.log('mouseClick to toggle popover : /' + this.id + '/');
            }

            function mouseEnter(e) {
                $('div[id^=card-main_m<?php echo $module->id; ?>_]').not(this).popover('hide');
                popover_trigger_id = this.id;
                setTimeout(function() 
                    {
                        $("#" + popover_trigger_id).popover('show');
                        console.log('mouseEnter to show popover : /' + popover_trigger_id + '/');
                    }, e.data.delay_time
                );
            }
